...
parser.add_argument('-s', '--server', help='the string representing the server to change the connection string to.')
...
def main(args):
	...
	if(args.excelFile):		
		changeConStringForExcel(args.excelFile, args.server)
	elif(args.fileList):
		...
		for filePathString in lines:
			...
			changeConStringForExcel(filePathString, args.server)
				

def changeConStringForExcel(filePathString, server):
	...
	os.chdir(tempDir)
	#raw_input('DEBUGGING: my directory is: %s' % os.getcwd()) 
	...
	if os.path.exists(strippedFilePathString):
		...
		try:
			...
			zfile.close()
			#extractedConFilePath = os.path.join(tempDir, 'xl\connections.xml')
			modifyConnection(server)
...
def modifyConnection(toString):
	try:
		...
		fromPattern = re.compile(r'Data Source=.*?;')
		toString = r'Data Source=' + escapeBackslashes(toString) + ';'
